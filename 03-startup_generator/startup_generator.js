...
var getRandomIdea = function () {
    var thisArrayRandom = thisArray[Math.floor(Math.random() * thisArray.length)];
    var thatArrayRandom = thatArray[Math.floor(Math.random() * thatArray.length)];    
    return (`${thisArrayRandom} for ${thatArrayRandom}`);
}


var randomIdea = [];

while (randomIdea.length < 5) {
    var idea = getRandomIdea();
    randomIdea.push(idea);
}

for (i = 0; i < randomIdea.length; i++) {
    console.log(`${numbers[i]} ${randomIdea[i]}`)
}